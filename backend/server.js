...
const { chatWithAI, extractTickers, analyzeNewsSentiment } =
...
const authenticateToken = (req, res, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, user) => {
    ...
    if (!existingUser) {
      ...
      return res
        .status(401)
        .json({ error: "User account not found. Please log in again." });
    }
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    // Prevent case-variance duplicates by checking lowercased values first
    const exists = db
      .prepare(
    ...
    const token = jwt.sign(
      { id: result.lastInsertRowid, username: normalizedUsername },
      JWT_SECRET,
      {
        expiresIn: "7d",
      },
  ...
  try {
    ...
    if (selectedTickerRaw) {
      ...
      const latestPublishedAt =
        newsPosts[0]?.news_published_at || newsPosts[0]?.created_at;
      const latestTs = latestPublishedAt
        ? new Date(latestPublishedAt).getTime()
        : 0;
      const stale =
        !latestTs ||
        Number.isNaN(latestTs) ||
        Date.now() - latestTs > NEWS_STALE_THRESHOLD_MS;
      ...
      if (needSync && shouldAutoSyncNews(selectedTicker)) {
        try {
          ...
          console.error(
            `Auto ingest failed for ${selectedTicker}:`,
            ingestError.message,
          );
        }
      }
...
async function fetchYahooChart(symbol, query) {
  const response = await fetch(
    `https://query1.finance.yahoo.com/v8/finance/chart/${symbol}?${query}`,
  );
  if (!response.ok) {
...
app.get("/api/stock/price/:symbol", async (req, res) => {
  try {
    const { symbol } = req.params;
    const { requestedSymbol, marketSymbol, proxyUsed } =
      getAssetProxyContext(symbol);
    ...
    if (date) {
      ...
      const period1 = toUnixSeconds(
        new Date(dayStart.getTime() - 3 * 24 * 60 * 60 * 1000),
      );
      const period2 = toUnixSeconds(
        new Date(dayEnd.getTime() + 3 * 24 * 60 * 60 * 1000),
      );
  ...
  for (const symbol of symbols) {
    ...
    if (existingPrices.count < 1) {
      try {
        ...
        console.error(
          `Error fetching prices for ${symbol} (market ${marketSymbol}):`,
          err,
        );
      }
    }
...
app.put("/api/user/onboarding", authenticateToken, (req, res) => {
  try {
    const investorType =
      typeof req.body.investorType === "string"
        ? req.body.investorType.trim()
        : "";
    const assetTypes = Array.isArray(req.body.assetTypes)
      ...
      : [];
    const riskTolerance =
      typeof req.body.riskTolerance === "string"
        ? req.body.riskTolerance.trim()
        : "";
    const decisionHorizon =
      typeof req.body.decisionHorizon === "string"
        ? req.body.decisionHorizon.trim()
        : "";
    const marketFocus =
      typeof req.body.marketFocus === "string"
        ? req.body.marketFocus.trim()
        : "";
    const baselineFlags = Array.isArray(req.body.baselineFlags)
      ? req.body.baselineFlags
          .map((item) => String(item || "").trim())
          .filter(Boolean)
      : [];
    const investmentAnchor =
      typeof req.body.investmentAnchor === "string"
        ? req.body.investmentAnchor.trim()
        : "";
    const completedAt =
      typeof req.body.completedAt === "string" ? req.body.completedAt : null;
...
app.post("/api/thesis/equities", authenticateToken, (req, res) => {
  try {
    ...
    const allocation =
      typeof req.body.allocation === "string" ? req.body.allocation.trim() : "";
    const thesis =
      typeof req.body.thesis === "string" ? req.body.thesis.trim() : "";
    const validity =
    ...
    if (error?.message?.includes("UNIQUE constraint failed")) {
      return res
        .status(409)
        .json({ error: "This symbol already exists in the selected bucket." });
    }
...
app.delete("/api/thesis/equities/:id", authenticateToken, (req, res) => {
  try {
    ...
    db.prepare("DELETE FROM thesis_equities WHERE id = ? AND user_id = ?").run(
      rowId,
      req.user.id,
    );
  ...
  try {
    ...
    if (panicPauseEvents.length > 0) {
      ...
      if (panicPauseEvents.length >= 2) {
        ...
        avgCoolingHours = Math.round(
          totalGap / (panicPauseEvents.length - 1) / (1000 * 60 * 60),
        );
      } else {
        avgCoolingHours = 24;
      }
    ...
    res.json({
      ruleAdherence:
        totalDecisions > 0 ? Math.round((honored / totalDecisions) * 100) : 0,
      totalDecisions,
      honored,
      overrides,
      panicPauses,
...
app.post("/api/thesis/decision-events/seed", authenticateToken, (req, res) => {
  try {
    ...
    const existing = db
      .prepare(
        "SELECT COUNT(*) as cnt FROM thesis_decision_events WHERE user_id = ?",
      )
      .get(userId);
    ...
    const events = [
      {
        type: "rule_honored",
        desc: "Held position through volatility per Rule #1",
        daysAgo: 2,
      },
      {
        type: "rule_honored",
        desc: "Rebalanced tech allocation below 60% threshold",
        daysAgo: 5,
      },
      {
        type: "rule_honored",
        desc: "Waited 24h before buying during VIX spike",
        daysAgo: 8,
      },
      {
        type: "rule_honored",
        desc: "Trimmed concentrated position to target weight",
        daysAgo: 12,
      },
      {
        type: "rule_honored",
        desc: "Logged rationale before executing trade",
        daysAgo: 15,
      },
      {
        type: "rule_honored",
        desc: "Paused new buys during elevated macro risk",
        daysAgo: 20,
      },
      {
        type: "rule_honored",
        desc: "Held through earnings despite pre-announcement anxiety",
        daysAgo: 25,
      },
      {
        type: "rule_honored",
        desc: "Reduced correlated positions per concentration rule",
        daysAgo: 30,
      },
      {
        type: "rule_honored",
        desc: "Followed stop-loss discipline on losing position",
        daysAgo: 35,
      },
      {
        type: "rule_honored",
        desc: "Reviewed thesis before adding to winner",
        daysAgo: 40,
      },
      {
        type: "rule_honored",
        desc: "Maintained cash buffer during drawdown",
        daysAgo: 50,
      },
      {
        type: "rule_override",
        desc: "Sold high-beta position before stop trigger during sharp drawdown",
        daysAgo: 24,
      },
      {
        type: "rule_override",
        desc: "Added to position during VIX > 20 despite pause rule",
        daysAgo: 45,
      },
      {
        type: "panic_pause",
        desc: "Market dropped 6.2% — paused for 48h",
        daysAgo: 14,
      },
      {
        type: "panic_pause",
        desc: "Flash crash scare — activated cooling-off period",
        daysAgo: 28,
      },
      {
        type: "panic_pause",
        desc: "Earnings miss triggered anxiety — paused trading",
        daysAgo: 42,
      },
      {
        type: "panic_pause",
        desc: "Portfolio drawdown 4% — took 24h break",
        daysAgo: 60,
      },
    ];
  ...
  try {
    ...
    if (!resolvedApiKey) {
      const storedSettings = db
        .prepare(
          "SELECT gemini_api_key FROM user_ai_settings WHERE user_id = ?",
        )
        .get(req.user.id);
    ...
    if (!resolvedApiKey) {
      return res.status(400).json({
        ...
        message:
          "Please set your Gemini API key in Add Key to use the AI Assistant.",
      });
    }
